...
export default function ZeroRiskReport() {
  ...
  const fetchZeroRisk = async (page = 1, limit = pagination.limit) => {
    setLoading(true);
    setError("");
    try {
  ...
  useEffect(() => {
    if (user && ["support", "admin"].includes(user.userType)) {
      fetchZeroRisk(pagination.currentPage, pagination.limit);
    }
  ...
  const handleSearch = (e) => {
    ...
    fetchZeroRisk(1, pagination.limit);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `Zero_Risk_Audit_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Zero Risk <span style={{ color: "#ffd700" }}>Audit</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Administrative terminal for tracking and auditing system-wide zero risk coverage and exposure.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        <SummaryCard title="Aggregate Risk Cap" value={Number(summary.totalCap).toFixed(2)} icon={ShieldCheck} color="#ffffff" />
        <SummaryCard title="Verified Coverage" value={Number(summary.totalUsed).toFixed(2)} icon={Zap} color="#ffd700" />
        <SummaryCard title="Exposed Value" value={Number(summary.pending).toFixed(2)} icon={Activity} color="#ff4444" />
      </div>
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "ZERO RISK", "CAPACITY", "UTILIZATION", "PENDING"].map(h => (
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", color: "#ffd700", fontWeight: "800" }}>{item.totalzerorisk || "0.00"}</td>
