...
export default function FiveXRewardsPage() {
  ...
  const fetch5xRewards = async (page = 1, limit = pagination.limit) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}api/support/deposits-greater?${params}`,
        {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `5xrewards_Report_${new Date().toISOString().split("T")[0]
        }.xlsx`;
      a.click();
  ...
  return (
    ...
    >
      ...
      >
        <h2>Positive Deposits Report</h2>
        <button
            ...
            {data.length > 0 ? (
              data.map((item, idx) => (
                ...
                >
                  ...
                  <td style={{ padding: "1rem", color: "#b3baff" }}>
                    {item.allTimeWithdrawal}
                  </td>
                  <td style={{ padding: "1rem", color: "#b3baff" }}>
                    {item.latestDepositAmount}
                  </td>
                  <td style={{ padding: "1rem", color: "#b3baff" }}>
                    {item.latestWithdrawalAmount}
                  </td>
                </tr>
              ))
            ) : (
