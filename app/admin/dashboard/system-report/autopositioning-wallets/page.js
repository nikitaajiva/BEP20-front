...
export default function FiveXRewardsPage() {
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}api/support/autopositioning-wallets/export?${params}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      a.href = url;
      a.download = `AutoPositioning_Report_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Fixed Auto <span style={{ color: "#ffd700" }}>Positioning</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Administrative terminal for managing and auditing fixed liquidity positioning protocols.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        <SummaryCard title="Aggregate Cap" value={Number(summary.totalCap).toFixed(2)} icon={Layers} color="#ffffff" sub="Lifetime allocation" />
        <SummaryCard title="System Used" value={Number(summary.totalUsed).toFixed(2)} icon={Zap} color="#ffd700" sub="Protocol utilization" />
        <SummaryCard title="Pending Records" value={Number(summary.pending).toFixed(2)} icon={ShieldCheck} color="#ff4444" sub="Awaiting validation" />
      </div>
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "LP", "ZERO RISK", "POSITIONING"].map(h => (
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", color: "#15C081", fontWeight: "800" }}>{item.lp}</td>
                  <td style={{ padding: "1.2rem 1rem", color: "#fff", fontWeight: "600" }}>{item.zeroRisk}</td>
                  <td style={{ padding: "1.2rem 1rem", borderTopRightRadius: "12px", borderBottomRightRadius: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#ffd700", fontWeight: "900" }}>
                      <Target size={14} /> {item.autopositioning}
                    </div>
                  </td>
                </tr>
              ))}
