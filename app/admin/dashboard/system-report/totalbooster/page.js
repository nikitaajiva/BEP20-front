...
export default function TotalBoosterPage() {
  ...
  const fetchBooster = async (page = 1, limit = pagination.limit) => {
    setLoading(true);
    setError("");
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/boost?${params}`, {
  ...
  useEffect(() => {
    if (user && ["support", "admin"].includes(user.userType)) {
      fetchBooster(pagination.currentPage, pagination.limit);
    }
  ...
  const handleSearch = (e) => {
    ...
    fetchBooster(1, pagination.limit);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/boost/export?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      a.href = url;
      a.download = `Booster_Report_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Total Booster <span style={{ color: "#ffd700" }}>Report</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Comprehensive administrative terminal for monitoring system boosters and utilization metrics.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        <SummaryCard title="Aggregate Cap" value={Number(summary.totalCap).toFixed(2)} icon={Layers} color="#ffffff" />
        <SummaryCard title="System Utilization" value={Number(summary.totalUsed).toFixed(2)} icon={Zap} color="#ffd700" />
        <SummaryCard title="Awaiting Processing" value={Number(summary.pending).toFixed(2)} icon={ShieldCheck} color="#ff4444" />
      </div>
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "TOTAL BOOSTER", "CAPACITY", "UTILIZATION", "PENDING"].map(h => (
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", borderBottom: idx === data.length - 1 ? "none" : "" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#ffd700", fontWeight: "900" }}>
                      <Cpu size={14} /> {item.boost || "0.00"}
                    </div>
                  </td>
                  <td style={{ padding: "1.2rem 1rem", color: "#fff", fontWeight: "600" }}>{item.cap}</td>
                  <td style={{ padding: "1.2rem 1rem", color: "#ffd700", fontWeight: "600" }}>{item.used}</td>
                  <td style={{ padding: "1.2rem 1rem", borderTopRightRadius: "12px", borderBottomRightRadius: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#00d9ff", fontWeight: "900" }}>
                      <TrendingDown size={14} /> {item.pending}
                    </div>
                  </td>
                </tr>
              ))}
