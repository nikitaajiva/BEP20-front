...
export default function FiveXRewardsReport() {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `5X_Rewards_Report_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            5X Rewards <span style={{ color: "#ffd700" }}>Report</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Administrative terminal for managing amplified incentive programs and 5X reward distribution.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        ...
        <SummaryCard title="Incentive Yield" value={Number(summary.totalUsed).toFixed(2)} icon={Zap} color="#ffd700" />
        <SummaryCard title="Awaiting Verification" value={Number(summary.pending).toFixed(2)} icon={ShieldCheck} color="#ff4444" />
      </div>
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "5X VALUE", "CAPACITY", "UTILIZATION", "PENDING"].map(h => (
                ...
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", borderBottom: idx === data.length - 1 ? "none" : "" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#ffd700", fontWeight: "900" }}>
                      <Star size={14} /> {item.xrewards || "0.00"}
                    </div>
                  </td>
