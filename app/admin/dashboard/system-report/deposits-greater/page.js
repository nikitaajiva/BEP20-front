...
export default function PositiveDepositsReport() {
  ...
  const fetchPositiveDeposits = async (page = 1, limit = pagination.limit) => {
    setLoading(true);
    setError("");
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/deposits-greater?${params}`, {
  ...
  useEffect(() => {
    if (user && ["support", "admin"].includes(user.userType)) {
      fetchPositiveDeposits(pagination.currentPage, pagination.limit);
    }
  ...
  const handleSearch = (e) => {
    ...
    fetchPositiveDeposits(1, pagination.limit);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/autopositioning-wallets/export?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      a.href = url;
      a.download = `Positive_Deposits_Audit_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Transaction <span style={{ color: "#ffd700" }}>Deposits</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Administrative terminal for auditing accounts with substantial deposit-to-withdrawal ratios.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        <SummaryCard title="Aggregate Inflow" value={Number(summary.totalCap).toFixed(2)} icon={ArrowUpRight} color="#ffffff" />
        <SummaryCard title="System Liquidity" value={Number(summary.totalUsed).toFixed(2)} icon={Wallet} color="#ffd700" />
        <SummaryCard title="Velocities" value={Number(summary.pending).toFixed(2)} icon={Activity} color="#00d9ff" />
      </div>
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "TRANSACTION DEP", "TRANSACTION WDL", "LATEST DEP", "LATEST WDL"].map(h => (
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", color: "#15c081", fontWeight: "900" }}>{item.allTimeDeposit || "0.00"}</td>
                  <td style={{ padding: "1.2rem 1rem", color: "#ff4444", fontWeight: "900" }}>{item.allTimeWithdrawal || "0.00"}</td>
                  <td style={{ padding: "1.2rem 1rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#ffd700", fontWeight: "700" }}>
                      <TrendingUp size={14} /> {item.latestDepositAmount || "0.00"}
                    </div>
                  </td>
                  <td style={{ padding: "1.2rem 1rem", borderTopRightRadius: "12px", borderBottomRightRadius: "12px" }}>
                     <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "rgba(255,255,255,0.4)", fontWeight: "700" }}>
                      <TrendingDown size={14} /> {item.latestWithdrawalAmount || "0.00"}
                    </div>
                  </td>
                </tr>
              ))}
