...
export default function FiveXRewardsPage() {
  ...
  const fetch5xRewards = async (page = 1, limit = pagination.limit) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}api/support/xaman?${params}`,
        {
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}api/support/xaman/export?${params}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      a.href = url;
      a.download = `XAMAN_Report_${new Date().toISOString().split("T")[0]
        }.xlsx`;
      a.click();
  ...
  return (
    ...
    >
      ...
      >
        <span>Total Xaman: {Number(summary.totalXaman).toFixed(6)}</span>
