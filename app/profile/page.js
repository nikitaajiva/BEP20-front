...
export default function ProfilePage() {
  ...
  // Auto-fill country and code from user or IP
  useEffect(() => {
    if (user) {
      const baseData = {
      ...
      fetch("https://ipapi.co/json/")
        .then((res) => res.json())
        .then((data) => {
          const ipCountry = data.country_name || "";
          ...
          setFormData((prev) => {
            ...
            if (
              prev.whatsappContact &&
              !prev.whatsappContact.startsWith(ipCode)
            ) {
              updated.whatsappContact = ipCode;
            }
            if (!prev.whatsappContact) {
              updated.whatsappContact = ipCode;
            }
  ...
  // ✅ Utility: Sanitize WhatsApp number
  function formatWhatsAppNumber(input) {
    ...
    if (digitsOnly.length > 25) {
      val = (val.startsWith("+") ? "+" : "") + digitsOnly.slice(0, 15);
    }
    ...
    setFormData((prev) => {
      ...
      if (initialFormData) {
        const isDifferent = Object.keys(updated).some(
          (key) => updated[key] !== initialFormData[key]
        );
          ...
          <div className={styles.inputGroup}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setFormData((prev) => ({
                  ...prev,
                  whatsappContact: formatted,
                }));
              }}
              className={styles.inputField}
              inputMode="numeric"
              pattern="^\+?[0-9]{10,25}$" // ✅ only allow + and 10–15 digits
              minLength={10}
              maxLength={25}
              required
