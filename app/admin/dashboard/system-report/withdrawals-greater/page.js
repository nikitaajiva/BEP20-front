...
export default function NegativeWithdrawalsReport() {
  ...
  const fetchNegativeWithdrawals = async (page = 1, limit = pagination.limit) => {
    setLoading(true);
    setError("");
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/withdrawals-greater?${params}`, {
  ...
  const handleSearch = (e) => {
    ...
    fetchNegativeWithdrawals(1, pagination.limit);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `Negative_Withdrawals_Audit_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Transaction <span style={{ color: "#ffd700" }}>Withdrawals</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Administrative terminal for auditing accounts with critical withdrawal-to-deposit variances.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        <SummaryCard title="Aggregate Outflow" value={Number(summary.totalCap).toFixed(2)} icon={ArrowDownLeft} color="#ffffff" />
        <SummaryCard title="Exposure Depth" value={Number(summary.totalUsed).toFixed(2)} icon={ShieldAlert} color="#ffd700" />
        <SummaryCard title="Variance Rate" value={Number(summary.pending).toFixed(2)} icon={Activity} color="#ff4444" />
      </div>
