...
export default function CommunityRewardsReport() {
  ...
  const fetchCommunityRewards = async (page = 1, limit = pagination.limit) => {
    setLoading(true);
    setError("");
    try {
  ...
  useEffect(() => {
    if (user && ["support", "admin"].includes(user.userType)) {
      fetchCommunityRewards(pagination.currentPage, pagination.limit);
    }
  ...
  const handleSearch = (e) => {
    ...
    fetchCommunityRewards(1, pagination.limit);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `Community_Rewards_Audit_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Community <span style={{ color: "#ffd700" }}>Rewards Audit</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Comprehensive administrative log of ecosystem-wide community incentive distributions.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        <SummaryCard title="Aggregate Fund" value={Number(summary.totalCap).toFixed(2)} icon={Coins} color="#ffffff" />
        <SummaryCard title="Verified Payouts" value={Number(summary.totalUsed).toFixed(2)} icon={Heart} color="#ffd700" />
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "COMMUNITY REWARDS"].map(h => (
                  <th key={h} style={{ padding: "0 1rem 1rem 1rem", color: "rgba(255,255,255,0.3)", fontSize: "0.7rem", fontWeight: "900", textAlign: h === "COMMUNITY REWARDS" ? "right" : "left", letterSpacing: "1px" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", borderTopRightRadius: "12px", borderBottomRightRadius: "12px", textAlign: "right" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#ffd700", fontWeight: "900", justifyContent: "flex-end" }}>
                      <Hash size={14} /> {item.TotalCommunityRewards || "0.00"}
                    </div>
                  </td>
                </tr>
              ))}
