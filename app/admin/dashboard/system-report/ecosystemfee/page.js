...
export default function EcosystemFeePage() {
  ...
  const fetchEcosystemFee = async (page = 1, limit = pagination.limit) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/system-report-ecofee-users?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json();
      if (!json.success) throw new Error(json.message || "Failed to fetch data");
      setData(json.data?.rows || []);
      setSummary({ totalCap: json.data?.grandTotal || 0, totalUsed: json.data?.grandTotal || 0, pending: 0 });
      setPagination((prev) => ({
        ...prev,
        totalItems: json.data?.totalUsers || 0,
        totalPages: Math.ceil((json.data?.totalUsers || 0) / (json.data?.pageSize || limit)),
        currentPage: json.data?.page || page,
        hasNextPage: (json.data?.page || page) < Math.ceil((json.data?.totalUsers || 0) / (json.data?.pageSize || limit)),
        hasPrevPage: (json.data?.page || page) > 1,
      }));
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/system-report-ecofee-users/export?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", color: "rgba(255,255,255,0.4)", fontFamily: "monospace", fontSize: "0.85rem" }}>{item.uhid || "N/A"}</td>
                  <td style={{ padding: "1.2rem 1rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#ffd700", fontWeight: "900" }}>
                      <Globe size={14} /> {item.amountStr || "0.00"}
                    </div>
                  </td>
