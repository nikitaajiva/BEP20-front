...
export default function TotalLPPage() {
  ...
  const fetchTotalLP = async (page = 1, limit = pagination.limit) => {
    setLoading(true);
    setError("");
    try {
  ...
  useEffect(() => {
    if (user && ["support", "admin"].includes(user.userType)) {
      fetchTotalLP(pagination.currentPage, pagination.limit);
    }
  ...
  const handleSearch = (e) => {
    ...
    fetchTotalLP(1, pagination.limit);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `TotalLP_Report_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Total LP <span style={{ color: "#ffd700" }}>Report</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Administrative terminal for tracking and analyzing total liquidity pool contributions.</p>
        </div>
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "TOTAL LP", "CAPACITY", "USED", "PENDING"].map(h => (
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem", color: "#15C081", fontWeight: "800" }}>{item.totallp || "0.00"}</td>
