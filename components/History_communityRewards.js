...
export const getCommunityTotals = (wallet, totals = {}) => {
  ...
  if (wallet !== "COMMUNITY_REWARDS") {
    ...
    return {
      ...
      totalRedeemed: "0.000000",
    };
  }

  const result = {
    totalDeposits: totals.currentAvailable || "0.000000",
    totalLPPoistioning: totals.totalLpositioning || "0.000000",
...
export default function HistoryTable({ filters, walletType = null }) {
  const { user, API_URL, logout } = useAuth();
  const [entries, setEntries] = useState([]);
  // const [totalLPPoistioning, setTotalLPPoistioning] = useState();
  ...
  const [pagination, setPagination] = useState({
    ...
    hasNextPage: false,
    hasPrevPage: false,
    limit: 250, // Show fewer entries on dashboard
  });
  ...
  const { totalDeposits, totalLPPoistioning, totalRedeemed } =
    getCommunityTotals(filters.wallet, totals);
  return (
    <div
      className="card p-4 mb-4"
      style={{
    ...
    >
      {/* <div className="USDT_total_headding">
        ...
        >
          <p className="content_p_deposits">Current Available:</p>
          <span>{totals.currentAvailable}</span>
        </div>
        <div
          style={{
            color: "#7FFF4C",
        ...
        >
          <p className="content_p_withdrawals">Total LP Positioned:</p>
          <span>{totals.totalLpositioning}</span>
        </div>
        <div
          style={{
            color: "#7FFF4C",
        ...
        >
          ...
          <span>{totals.totalRedeemed}</span>
        </div>
      </div> */}
      <div
          ...
          <tbody>
            {entries.map((entry, index) => (
              ...
              >
                ...
                >
                  {parseFloat(entry.amount).toFixed(6)} USDT
                </td>
                <td
                  style={{
                    padding: "1.25rem 1rem",
                ...
                {entry.eventType === 'ECOSYSTEM_FEE'
  ?  <RefIdDisplay refId=''/>
  : <RefIdDisplay refId={entry.refId} />}
                </td>
                {/* <td
                  style={{
                    padding: "1.25rem 1rem",
                ...
                >
                  {parseFloat(entry.available_balance).toFixed(6)}
                </td> */}
              </tr>
            ))}
          </tbody>
