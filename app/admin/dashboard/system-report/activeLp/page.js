...
export default function ActiveLpReport() {
  ...
  const fetchActiveLp = async (page = 1, limit = pagination.limit) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}api/support/activeLp?${params}`, {
  ...
  const handleExport = async () => {
    try {
      ...
      if (toDate) params.append("toDate", toDate);
      if (date) params.append("date", date);
      const res = await fetch(`${API_BASE_URL}api/support/activeLp/export?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      a.href = url;
      a.download = `Active_LP_Report_${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
  ...
  return (
    <div style={{ background: "#060606", minHeight: "100vh", padding: "2rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "2.4rem", fontWeight: "900", color: "#fff", margin: 0, letterSpacing: "-1.5px" }}>
            Active LP <span style={{ color: "#ffd700" }}>Terminal</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.95rem", marginTop: "6px" }}>Real-time audit of active liquidity positions and protocol engagement across the ecosystem.</p>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginBottom: "2.5rem" }}>
        <SummaryCard title="Aggregate Liquidity" value={Number(summary.totalCap).toFixed(2)} icon={Activity} color="#ffffff" />
        <SummaryCard title="Active Protocol Yield" value={Number(summary.totalUsed).toFixed(2)} icon={BarChart3} color="#ffd700" />
        <SummaryCard title="Awaiting Validation" value={Number(summary.pending).toFixed(2)} icon={ShieldCheck} color="#ff4444" />
      </div>
      ...
      <div style={{ ...glassStyle, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
            <thead>
              <tr>
                {["ENTITY", "UHID", "LP VALUE", "AUTO POSITIONING", "ACTIVE LP"].map(h => (
              ...
              {data.map((item, idx) => (
                <tr key={idx} style={{ background: "rgba(255,255,255,0.02)", transition: "all 0.2s ease" }} className="table-row">
                  ...
                  <td style={{ padding: "1.2rem 1rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#ffffff", fontWeight: "900" }}>
                      {item.lp || "0.00"}
                    </div>
                  </td>
                  <td style={{ padding: "1.2rem 1rem" }}>
                  ...
                  <td style={{ padding: "1.2rem 1rem", borderTopRightRadius: "12px", borderBottomRightRadius: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                       <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#00d9ff", fontWeight: "900" }}>
                         <Workflow size={14} /> {item.activeLp || "0.00"}
                       </div>
                    </div>
                  </td>
                </tr>
