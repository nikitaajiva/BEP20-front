  ...
  return (
    ...
    >
      ...
      <div className={styles.cardHeader}>
        <div className={styles.cardIconWrap} style={{ background: `${accentColor}12`, color: accentColor }}>
          {Icon ? <Icon size={17} /> : <BarChart2 size={17} />}
        </div>
        {detailHref && (
          <a href={detailHref} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}
            style={{
      ...
      <div className={styles.cardValue}>{fmt(value)} <span className={styles.cardUnit}>USDT</span></div>
      {count != null && (
        <div className={styles.cardBadge} style={{ color: accentColor, background: `${accentColor}10`, border: `1px solid ${accentColor}20` }}>
          <Users size={10} /> {count} users
        </div>
      )}
  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        fontWeight: 800, fontSize: 12, letterSpacing: 0.5, transition: "all 0.25s ease",
...
function ChartCard({ title, badge, children, flex = "1 1 400px" }) {
  return (
    <div style={{
      ...
      borderRadius: 20, padding: "24px 26px", backdropFilter: "blur(16px)", transition: "all 0.3s ease",
    }}>
...
const DETAIL_MAP = {
  ...
  "Cascade Analytics": "/admin/dashboard/community-rewards",
  "X-Performance Rewards": "/admin/dashboard/x1-bonus-report",
  "Community Reward Pool": "/admin/dashboard/community-rewards-report",
  "Withdrawal Errors": "/admin/dashboard/system-report/withdrawal-errors",
};

export default function SystemReportContent() {
  ...
  if (error) return <div style={{ ...centeredStyle, color: "#f43f5e", fontSize: 14, fontWeight: 700 }}>⚠ {error}</div>;
  ...
  const {
    ...
    onChainDepositsToday, onChainWithdrawalsToday, ecosystemFeesToday,
  ...
  const renderWallet = () => (
    <div className={styles.contentWrap}>
      <ChartCard title="Wallet Balances Breakdown" badge="Lifetime Totals">
        <div style={{ height: 280 }}><Bar data={walletChartData} options={chartOpts()} /></div>
      </ChartCard>
      ...
      <div className={styles.reportGrid}>
        ...
        ].map((c) => <MetricCard key={c.label} {...c} detailHref={DETAIL_MAP[c.label]} />)}
      </div>
    </div>
  );
    ...
    <div className={styles.contentWrap}>
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        <ChartCard title="Transactions Volume Breakdown" badge="All Time" flex="1 1 500px">
          <div style={{ height: 280 }}><Bar data={onChainChartData} options={chartOpts()} /></div>
        </ChartCard>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 6 }}>
            {[
              ...
              <div key={item.label} style={{ padding: "12px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 12, borderLeft: `3px solid ${item.color}` }}>
                ...
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 3 }}>{item.tx} txns · {item.users} users</div>
              </div>
            ))}
          </div>
        </ChartCard>
      ...
      <div className={styles.reportGrid}>
        {[
          ...
          { label: "Withdrawal Errors", value: report.withdrawalErrorCount || 0, sub: "Failed on-chain attempts", accentColor: "#ff4444" },
        ].map((c) => <MetricCard key={c.label} {...c} detailHref={DETAIL_MAP[c.label]} />)}
      </div>
    </div>
  );
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        <ChartCard title="Allocation Share" badge="Protocol Split" flex="0 0 360px">
          <div style={{ height: 260, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative" }}>
            <Doughnut data={donutData} options={{ responsive: true, maintainAspectRatio: false, cutout: "72%", plugins: { legend: { display: false }, tooltip: TOOLTIP } }} style={{ position: "absolute", inset: 0 }} />
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginTop: 16 }}>
            {["#ffd700", "#10b981", "#6366f1", "#f97316", "#f43f5e", "#8b5cf6"].map((c, i) =>
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.4)" }}>
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: c }} /> {donutData.labels[i]}
              </div>
            )}
          </div>
        </ChartCard>
        <ChartCard title="Allocation Amounts" badge="Lifetime" flex="1 1 400px">
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 6 }}>
            {[
              { label: "Daily Yield Pool", value: distributedLpRewards, color: "#ffd700" },
  ...
  const renderDaily = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      {dailyRewards?.date ? (
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 20px", background: "rgba(255,215,0,0.05)", border: "1px solid rgba(255,215,0,0.12)", borderRadius: 14, width: "fit-content" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 15, fontWeight: 900, color: "#ffd700", marginTop: 2 }}>{new Date(dailyRewards.date).toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</div>
            </div>
          </div>
          {dailyChartData && (
            <ChartCard title="Daily Reward Trends" badge="Last Run">
              <div style={{ height: 280 }}><Line data={dailyChartData} options={chartOpts()} /></div>
            </ChartCard>
          )}
          <div style={{ padding: "20px 26px", background: "linear-gradient(135deg,rgba(255,215,0,0.08),rgba(255,165,0,0.04))", border: "1px solid rgba(255,215,0,0.2)", borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
            ...
            <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(255,215,0,0.12)", display: "flex", alignItems: "center", justifyContent: "center" }}><BarChart2 size={22} color="#ffd700" /></div>
          </div>
      ...
      ) : <div style={{ textAlign: "center", padding: "60px 20px", color: "rgba(255,255,255,0.25)", fontSize: 14, fontWeight: 700 }}>No daily distribution data available.</div>}
    </div>
  );
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          ...
          <h1 className={styles.title}>System <span>Report</span></h1>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.25)", margin: "6px 0 0", fontWeight: 500 }}>Full protocol analytics across all system wallets and reward pools</p>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginTop: 15 }}>
          <ExportUserReportButton />
          <button onClick={fetchReport} className={styles.refreshBtn} style={{ display: "flex", alignItems: "center", gap: 7, padding: "9px 18px", borderRadius: 12, border: "1px solid rgba(255,215,0,0.15)", background: "rgba(255,215,0,0.05)", color: "rgba(255,215,0,0.7)", cursor: "pointer", fontSize: 12, fontWeight: 800, transition: "all 0.2s ease" }}>
...
const centeredStyle = { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: 12 };
